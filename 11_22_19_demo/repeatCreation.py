...
while True:
    for x in range(random.randint(1, 5)):
        ...
        message = ""
        if user_type == 0:
            results = table.aggregate([{'$match': {'TYPE': "STUDENT"}}, { "$sample": { "size": 1 } }])
            data = results.next()
            pid = data["PID"]
            fiuEmail = data["EMAIL"]
            major = data["MAJOR"]
            mac = data["MAC"]
            message = building + " " + ip + " " + mac + " " + pid + " " + \
            str(datetime.now().isoformat()) + " " + fiuEmail + " STUDENT " + major + "\n"
        elif user_type == 1:
            results = table.aggregate([{'$match': {'TYPE': "FACULTY"}}, { "$sample": { "size": 1 } }])
            data = results.next()
            pid = data["PID"]
            fiuEmail = data["EMAIL"]
            ...
            results = table.aggregate([{'$match': {'PID': None}}, { "$sample": { "size": 1 } }])
            pid = "None"
            data = results.next()
            mac = data["MAC"]
            fiuEmail = data["EMAIL"]
